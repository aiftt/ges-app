...
计划：@plan.md，按照计划推进，完成一个组件之后继续下一个直到所有组件完成，记得更新计划文档
...
1. table
   - 排序的两个箭头点击没反应，不是让你改变他们的布局（就保持上下布局），就解决功能不生效问题就行
   - 要支持设置最大宽度，如果宽度不够水平方向应该出现滚动条，这个时候表格列的内容尽可能展示全(在没有设置列宽度的情况下)，好好理解我的要求进行改进
2. upload 没有 demo
3. 优化下 pages/demo/components.vue 页面，左侧菜单，和内容区的滚动逻辑分开处理，不然内容区滚动菜单也跟着动体验不太好
