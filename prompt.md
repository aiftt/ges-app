...
1. VirtualSelect 你这实现有很大问题，你这有10w个数据不得创建10w个dom元素？？？参考element-plus的实现改造优化下，多选模式下不要选一个就关闭下拉框
2. InfiniteScroll 无限滚动 也是一样的问题，

这种无限滚动的实现逻辑，请参考 element-plus 实现，但是肯定不是创建无限个 dom 元素，我的理解和思路应该是从 固定一定的元素然后通过更新数据方式，仅供参考具体实现你自己好好思考下
