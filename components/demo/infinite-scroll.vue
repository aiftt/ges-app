...
function loadMoreData() {
  ...
  setTimeout(() => {
    if (currentPage.value <= totalPages) {
      // 模拟数据
      const newItems = Array.from({ length: pageSize.value }, (_, index) => {
        const itemIndex = (currentPage.value - 1) * pageSize.value + index + 1
        return {
          id: itemIndex,
          title: \`项目 \${itemIndex}\`,
...
// 加载水平滚动数据
function loadHorizontalData() {
  if (horizontalLoading.value || horizontalNoMore.value) return
...
const customContainerCode = `<template>
...
// 加载自定义容器数据
function loadContainerData() {
  if (containerLoading.value || containerNoMore.value) return
...
const virtualScrollCode = `<template>
  <ui-infinite-scroll
...
// 加载虚拟滚动数据
function loadVirtualScrollData() {
  if (virtualScrollLoading.value || virtualScrollNoMore.value) return
...
const performanceInfoCode = `<template>
...
<template>
  <div class="mx-auto max-w-6xl">
    ...
    <p class="mb-6 text-gray-500 dark:text-gray-400">
      无限滚动组件用于在用户滚动到底部时自动加载更多数据，提供更流畅的浏览体验。
    </p>

    <!-- 基础用法 -->
    <ui-demo
      title="基础用法"
      description="滚动到底部时自动加载更多数据。"
      :code="basicCode"
    >
      <div class="border rounded bg-white p-4 dark:bg-gray-800">
        <div class="mb-4 flex justify-end">
    ...
    <ui-demo
      title="自定义提示内容"
      description="可以自定义加载中、无更多数据和错误状态的提示内容。"
      :code="customTipsCode"
    >
      <div class="border rounded bg-white p-4 dark:bg-gray-800">
        <ui-infinite-scroll
    ...
    <ui-demo
      title="虚拟滚动"
      description="使用虚拟滚动技术高效渲染大量数据，同时支持无限加载。"
    ...
    <ui-demo
      title="虚拟滚动性能说明"
      description="虚拟滚动技术可以大幅提升大数据量渲染的性能。"
      :code="performanceInfoCode"
    >
      <div class="border rounded bg-white p-4 dark:bg-gray-800">
        <p class="mb-2 text-gray-600 dark:text-gray-400">
          上面的演示加载了 {{ virtualScrollData.length }} 条数据，但实际只渲染了可见区域的少量DOM元素。
